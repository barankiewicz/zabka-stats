...
backend/                 - code + API (chapter 2)
  ...
  api/                   - decomposed routers (locations, history, admin, geo_router, ecology_router, stats_router, spatial_router, demographics)
                           plus fact_pages.py (/fakt/{slug} share pages + OG images, top-level, not under /api)
  og_image.py             - Pillow-rendered per-fact OG preview PNGs (used by fact_pages.py)
  assets/fonts/           - IBM Plex Sans / JetBrains Mono .ttf, bundled for og_image.py (Pillow needs local font files)
...
- **Shareable Fact Pages** (`backend/api/fact_pages.py`, not under `/api` - these are
  ...
  - `GET /fakt/{slug}` -> Serves the SPA `index.html` with per-fact `<title>`,
    description, canonical URL, and OG/Twitter meta tags injected (regex-substituted
    into the built `dist/index.html`), so a social/search preview shows that one
    stat instead of the generic homepage card. Valid slugs: `pustka-bieszczadzka`,
    `samotna-zabka`, `najstarsza-zabka`, `zielonej-zabki`, `mediana-odleglosci`;
...
their own crawlable URL under `/fakt/{slug}`, served server-side by
`backend/api/fact_pages.py` - see [Chapter 2 API](#4-api) for the two endpoints. Landing
on one of these URLs still loads the same SPA, but with per-fact `<title>`, description,
canonical link, and OG/Twitter tags (and its own `/fakt/{slug}/og.png` preview image), and
`main.js`'s `initFactDeepLink()` scrolls/flies straight to that fact once the SIEĆ tab
(where all five live) has rendered - `kraniec.js`'s `selectFact()` queues the call if the
Atlas map itself hasn't finished its own lazy build yet. `sitemap.xml` lists all five URLs.
