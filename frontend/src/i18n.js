...
export const translations = {
  en: {
    ...
    map_growth_sub: "Each dot is a store, appearing in its opening year. Alongside is the opening calendar month-by-month - the slider drives the timeline of them both.",
    calendar_aria_label: "Calendar of store openings month-by-month, 1998-{{STAT_DATA_YEAR_MAX}}. Lighter fields indicate more openings in a given month.",
    slider_year_label: "Year of network expansion",

    // Growth Chart
    chart_growth_title: "How the wave grew stronger every year",
    chart_growth_sub: "Bars: new stores that year (left axis). Line: YoY change % (right axis).",
    ...
    origin_old_note_suffix: " - and still going strong",
    origin_new_kicker: "Newest Żabka",
    origin_new_note: "opened",
    origin_new_note_suffix: " - the newest point on the map",

    // Fingerprint Card
    fingerprint_title: "Compass - growth trend, N-E-S-W-N direction",
    fingerprint_sub: "Growth data unrolled from the polar layout: X-axis is direction (N-E-S-W-N), Y-axis is year. Each ring represents one year, and the bulge indicates the dominant direction of expansion. Hover for details.",
    ...
    // Bridge Cards
    bridge_expansion: "Expansion direction year-by-year is one story. The other: how many stores and where. {{STAT_LEADER_ABSOLUTE_VOIV}} leads in absolute numbers - but {{STAT_LEADER_PERCAPITA_VOIV}} wins per capita.",
    bridge_econ_text: "The network looks evenly spread. The data tells a different story.",
    bridge_econ: "Wealthier powiats have more stores. The West closes on Sundays, while the rest does not. 32 Żabkas have a distance to the nearest Żabka equal to 0. Let's see how it is possible and how Żabka fits into the image of contemporary Poland.",
    bridge_cta_spoleczenstwo: "Żabka & Poland",

    // Najwiecej Zabek (Granular)
    gran_title: "Most Żabkas - powiats",
    gran_sub: "active stores count",
    gran_dim_woj: "Voivodeship",
    gran_dim_powiat: "Land powiats",
    ...
    edge_kpi_parks_sub: "stores in national parks",
    ...
    edge_kpi_frogrecord_tile: "Amphibian sighting record - show on map",
    ...
    edge_kpi_farthestfrog_tile: "Farthest from frog sighting - show on map",
    ...
    lead_totals_template: "<b>{total}</b> stores in <b>{powiats}</b> powiats. In two chapters, we check if network density follows <b>wealth</b> and <b>employment</b> - and what history the numbers are really telling.",
    resort_sub_per1k: "stores by stores per 1,000 registered residents - sea and mountains beat the rest of the country",
    resort_sub_perkm2: "stores by stores per km² - large cities win here",
    ...
    map_zoom_hint_mouse: "ctrl + scroll to zoom",
    ...
    ep_frog_note: "Żabka on Green Frog Street - a perfect marketing coincidence (Żabka - a little frog in Polish).",
    ...
    ep_lowest_eyebrow: "The only one below sea level",
    ep_lowest_city: "Gdańsk (port), Pomorskie",
    ep_lowest_street: "Przełom 12",
    ep_isolated_panel: "Most isolated store - show on map",
    ep_isolated_eyebrow: "Farthest from the neighbor",
    ep_isolated_city: "Michałowo, Podlaskie",
    ep_isolated_street: "Białostocka 2",
    ep_zerofrog_panel: "Store without any frog sightings nearby - show on map",
    ep_zerofrog_eyebrow: "Żabka without any frog sightings nearby",
    ...
    coverage_suffix_powiat: "powiats have a Żabka",
    coverage_suffix_city: "cities have a Żabka",
    coverage_suffix_gmina: "gminas have a Żabka",

    // City Gap (cities with zero Żabki)
    citygap_pop_unit: " res.",

    // Bubble Chart
    bubble_title: "What cities make up the network",
    bubble_sub: "Bubble size represents the number of stores. Drag to pan; Ctrl + scroll to zoom. Smaller cities fall into 'Others'.",
    ...
    hero_lede_spol: "In Poland, Żabka feels right at home. In nearly 30 years, it has become a cultural icon of the country. Let's see where else it fits into Polish culture. Or does it corrupt it?",
    ...
    spol_kpi_gminy_kicker: "Gmina coverage",
    spol_kpi_gminy_sub: "of gminas have at least one Żabka",
    ...
    inpost_sub: "A duel between two giants of Polish public space. Both serve different needs - Żabka is shopping, InPost is parcel pickup - but both compete for every square meter of the street.",
    legend_Żabka_100k: "Żabkas/100k",
    legend_inpost_100k: "InPosts/100k",
    ...
    // Elevation
    ele_title: "From the port in Gdańsk to the peak in the Tatry",
    ele_sub: "Distribution of active store elevation above sea level, in 50m buckets.",
    ele_caveat: "The extremes are single points: a store at the port in Gdańsk sits below sea level, while one in Kościelisko under Tatry sits highest in the whole network. 90% of stores fall between the P5 and P95 lines (Percentiles).",

    // Streets
    streets_title: "Streets with the most Żabkas",
    streets_sub_prefix: "Specific street and city, not just the name - a total of ",
    streets_sub_suffix: " stores",

    // Resort stores
    resort_title: "Most Żabkas per capita? Resorts.",
    resort_sub: "stores by stores per 1,000 registered residents - sea and mountains beat the rest of the country",
    ...
    // Nav links to the dedicated FAQ and methodology pages - both pages
    // carry the full i18n toggle (data-t/JSON-LD), same as this SPA
    nav_faq_link: "FAQ",
    nav_methodology_link: "Methodology",
    ...
    faq_og_image_alt: "FAQ about the Żabka network in numbers - facts, sources and common misconceptions",
    faq_h1: "FAQ",
    ...
    faq_a_farthest: "The point farthest from any Żabka in Poland is located in the Bieszczady Mountains, about {{STAT_VOID_DISTANCE_KM}} km in a straight line from the nearest store. This is practically the middle of Połoniny Wetlińskie.",
    ...
    faq_a_closes: "Yes, it does. The issue lies elsewhere: our growth history chart (on the dashboard's Network tab) counts openings only for stores that are active *today*. A store that opened in 2015 and closed in 2022 simply does not exist in this chart. This makes the early years look weaker than they actually were (see \"survivorship bias\" below).",
    faq_q_every_city: "Is there a Żabka in every town/city in Poland?",
    faq_a_every_city: "No, but it is close - over {{STAT_GMINY_COVERAGE_PCT}}% of gminas have at least one Żabka, and powiat coverage is virtually complete ({{STAT_POWIAT_COVERED}} out of {{STAT_POWIAT_TOTAL}} land powiats have at least one store). Places without Żabkas are mostly small, scattered cities. Check the dashboard to find out which exactly!",
    faq_sec_sources: "Where the Data Comes From",
    faq_q_source_origin: "Where does this data come from?",
    faq_a_source_origin: "The main source is the public store locator JSON file on <code>Żabka.pl</code> - the same one used by the store locator search engine on their website. We enrich it with data from GUS BDL (wages, unemployment, population), GBIF (amphibian observations), InPost ShipX (parcel lockers), and GUGiK (administrative boundaries, geocoding, land elevation). A full description of the sources, the entire ETL pipeline, and the list of known limitations can be found on the <a href=\"/methodology.html\">methodology page</a>, as well as in the repo.",
    faq_q_update_freq: "How often is the data updated?",
    faq_a_update_freq: "Daily, via a cron pipeline at 3:00AM. GUS economic data (wages, unemployment, population) is updated less frequently, as GUS itself publishes it once a year.",
    ...
    faq_a_official: "No. ŻabkaBoard is an independent fan/analytical project based on public data, not affiliated with Żabka. The name \"Żabka\" and related trademarks belong to their respective owners.",
    faq_q_why: "Why did you build this?",
    faq_a_why: "Two reasons, honestly. I wanted a dataset people could actually use, and I wanted to learn a pile of things by doing rather than reading about them - hosting my own service on a Linux VPS, building a fast API in Litestar, working with DuckDB and Polars, writing an ETL pipeline that enriches data instead of just moving it around, visualizing GIS data (GeoJSON, point-in-polygon, coordinate systems), and making a site that's properly multilingual and fast on both ends. The full list is in the <a href=\"https://github.com/barankiewicz/zabka-stats#10-my-goals\" target=\"_blank\" rel=\"noopener\">README on GitHub</a>.",
    faq_sec_pitfalls: "Common Pitfalls & Misinterpretations",
    faq_pitfalls_note: "This section exists because statistical data is easily bent to support whatever story we want to hear. Below are popular pitfalls that are easy to stumble on when looking at this dashboard - and why conclusions that seem \"obvious at first glance\" can sometimes be false.",
    faq_q_econ_correlation: "Does high correlation on economic maps mean that more wealth causes more Żabka stores?",
    faq_a_econ_correlation: "No - and this is the single most important pitfall on the entire dashboard. The maps in the \"Żabka & Poland\" section show **correlation**: the deviation of network density from the trend determined by wages or unemployment in a given powiat. The correlation coefficient r (e.g., r = +{{STAT_R_SALARY}} for wages) describes the strength of this statistical relationship - not a causal mechanism. It could just as easily be the other way around (more stores boosting the local economy), or - most likely - both variables depend on a third factor: population density and urbanization. Wealthier powiats are usually also more densely populated and more urbanized, and that is what actually attracts convenience chains - not the mere presence of money.",
    faq_q_amphibians_density: "Does a higher number of amphibian observations in GBIF near a store mean more frogs live there?",
    faq_a_amphibians_density: "Not at all! The record holder (over {{STAT_AMPHIBIAN_RECORD_COUNT}} observations within a 5 km radius) is a store in Ursynów, Warsaw - a densely populated powiat with parks, not a nature reserve. GBIF data consists of citizen science reports: they reflect the density of *observers* with smartphones, not just the density of amphibians. Pristine, sparsely populated areas (Bieszczady, Bialowieza Forest) might have more frogs in reality but fewer reports - simply because fewer people are looking there and uploading observations.",
    faq_q_growth_bias: "Does the network growth chart since 1998 show the full history of openings?",
    faq_a_growth_bias: "No. The chart counts openings exclusively for stores that are active *today*. A store that opened in 2003 and closed in 2015 is invisible - as if it never existed. This makes the early years (1998-2010) look weaker than they actually were, because part of that cohort has already dropped out. The curve you see is a \"history of the winners,\" not the complete history of the network.",
    ...
    // Methodology Page
    meth_title: "Methodology and Data Sources - ŻabkaBoard",
    ...
    meth_og_image_alt: "Żabka data methodology - sources, definitions and limitations",
    meth_h1: "Methodology",
    ...
    meth_disclaimer: "ŻabkaBoard is an independent fan/analytical project based on public data. It is not affiliated with Żabka. The name \"Żabka\" and related trademarks belong to their respective owners.",
    ...
    meth_source_map_desc: "The structure of Poland's territorial division (four levels: voivodeships, powiats/powiaty, and cities and gminy) is built directly from the official GUS BDL and TERYT registers as the first step of the ETL process. Stores and parcel lockers are mapped to this hierarchy relationally based on their location names, using GUGiK's official Universal Geocoding Service to unambiguously resolve smaller towns and villages, with a spatial match fallback. This eliminates the need for simplified GeoJSON files and slow point-in-polygon checks.",
    meth_source_econ_title: "Economy - GUS BDL",
    meth_source_econ_desc: "Wages, unemployment, and population at the powiat level are sourced from the BDL (variables 64428, 60270, 72305). We download them once a year. Powiat names must be normalized - GUS likes to add prefixes (<code>Powiat m.</code>), temporal suffixes (<code>since 2023</code>), and rename powiats (<code>jeleniogórski</code> became <code>karkonoski</code> in 2021). We clean all of this before matching. Accuracy: the data comes from the year GUS published it, not the current month.",
    ...
    meth_etl_step4: "<strong>Diff.</strong> We compare the new snapshot with the previous one: new <code>store_id</code> records indicate openings, while missing ones indicate closures. The result is saved as a history of changes implemented by a deleted_at column in the locations table.",
    meth_etl_step5: "<strong>Database Save.</strong> Data is written to the DuckDB database. New columns are added using <code>ALTER TABLE ADD COLUMN IF NOT EXISTS</code>",
    meth_etl_step6: "<strong>Cache Purge.</strong> After a successful ETL run, the Redis cache is cleared. The backend rebuilds cache entries on the next request. Redis is optional - if it is unavailable (for example in local environments), the backend works without it.",
    ...
    meth_warn_item2: "Powiat populations are annual data from GUS, not real-time estimates.",
    ...
    meth_warn_item4: "Opening hours are sourced from Żabka's system, not verified IRL.",
    meth_warn_item5: "The main data source includes <strong>only currently active stores</strong>. So we don't have historical data - the oldest Żabka doesn't necessarily have to be the oldest in history, because it's only the oldest out of the <strong>active ones</strong>.",
    ...
    fact_desc_west: "The western extreme: Cedynia on the Odra River, a few kilometers from Germany.",
    ...
    aria_gmina_metric: "Gmina metric",
    ...
    chart_gmina_lead_aria: "Bar chart: gminas with the most stores per resident or per km², by the selected metric.",
    gran_dim_gmina: "Gminas",
    gran_word_gmina: "gminas",
  ...
  pl: {
    ...
    // Siec Tab - Hero
    hero_eyebrow_siec_snapshot: "Atlas Żabek - migawka {date}",
    hero_eyebrow_siec_data: "Atlas Żabek - dane {year}",
    ...
    calendar_aria_label: "Kalendarz otwarć sklepów miesiąc po miesiącu, 1998-{{STAT_DATA_YEAR_MAX}}. Jaśniejsze pola oznaczają więcej otwarć w danym miesiącu.",
    ...
    bridge_cta_spoleczenstwo: "Żabka a Polska →",

    // Najwiecej Zabek (Granular)
    gran_title: "Najwięcej Żabek - powiaty",
    gran_sub: "liczba aktywnych sklepów",
    gran_dim_woj: "Woj.",
    gran_dim_powiat: "Pow. ziemskie",
    ...
    ele_caveat: "Skrajności to pojedyncze punkty: sklep w porcie w Gdańsku leży poniżej poziomu morza, a w Kościelisku pod Tatrami najwyżej w całej sieci. 90% sklepów mieści się między liniami P5 i P95 (Percentyle).",
    ...
    // Linki do stron FAQ i metodyki - obie strony mają pełny przełącznik
    // językowy (data-t/JSON-LD), tak jak główny SPA
    nav_faq_link: "FAQ",
    ...
    // FAQ Page
    faq_title: "FAQ - najczęstsze pytania o sieć Żabka i dane",
    faq_meta_desc: "Ile jest Żabek w Polsce, gdzie jest ich najwięcej, jak daleko do najbliższej - i dlaczego korelacja na mapach ekonomicznych nie znaczy przyczynowości. Pytania i odpowiedzi.",
    ...
    faq_a_closes: "Tak, zamyka. Problem jest inny: nasz wykres historii wzrostu liczy otwarcia tylko dla sklepów, które są aktywne <em>dziś</em>. Sklep, który otworzył się w 2015 i zamknął w 2022, po prostu nie istnieje w tym wykresie - jakby nigdy go nie było. To sprawia, że wcześniejsze lata wyglądają na słabsze niż realnie były (patrz \"survivorship bias\" niżej). Zamykanie sklepów zaczęliśmy śledzić dopiero, odkąd ten projekt działa - wraz z czasem danych przybędzie, ale na razie jest ich na tyle mało, że ich nie prezentujemy.",
    faq_q_every_city: "Czy Żabka jest w każdym mieście w Polsce?",
    faq_a_every_city: "Nie, ale jest blisko - ponad {{STAT_GMINY_COVERAGE_PCT}}% gmin ma co najmniej jedną Żabkę, a pokrycie powiatów jest praktycznie kompletne ({{STAT_POWIAT_COVERED}} z {{STAT_POWIAT_TOTAL}} powiatów lądowych ma przynajmniej jeden sklep). Miejsca bez Żabki to głównie małe, rozproszone gminy wiejskie - zobacz w Atlasie Krańców, które!",
    ...
    faq_a_update_freq: "Codziennie cronem o 3:00. Ekonomiczne dane GUS (zarobki, bezrobocie, populacja) aktualizują się rzadziej, bo GUS sam publikuje je raz w roku.",
    ...
    faq_a_sunday_strategy: "Dane pokazują tylko wynik (flaga <code>open_sunday</code> per sklep), nie przyczynę. Nie chcę spekulować na temat przyczyn.",
    faq_q_completeness: "Czy ten zbiór danych jest kompletny i wolny od brakujących wartości?",
    faq_a_completeness: "Nie. W danych jest szereg quality issues. Około {{STAT_UNDATED_STORES}} sklepów nie ma daty otwarcia w źródle. Populacje powiatów to dane roczne z GUS, nie aktualne szacunki. Pełna lista ograniczeń jest na <a href=\"/methodology.html#czego-nie-gwarantujemy\">stronie metodyki</a>.",
    ...
    // Methodology Page
    meth_title: "Metodyka i źródła danych - Żabkozbiór",
    ...
    meth_source_frogs_desc: "Obserwacje płazów z Global Biodiversity Information Facility (klasa Amphibia, <code>taxonKey=131</code>) w Polsce. Mamy około {{STAT_GBIF_TOTAL}} rekordów ze współrzędnymi. Za pomocą BallTree (haversine) zliczamy obserwacje w promieniu 5 km od każdego sklepu. Najbardziej żabia Żabka ma {{STAT_AMPHIBIAN_RECORD_COUNT}} obserwacji - i to w Ursynowie, nie w żadnym parku narodowym.",
    meth_source_neighbor_title: "Sąsiedztwo - obliczenia lokalne",
    meth_source_neighbor_desc: "Odległość do najbliższej innej Żabki obliczamy lokalnie przez BallTree (haversine, k=2). Najbardziej odosobniony sklep ma {{STAT_ISOLATED_MAX_KM}} km do najbliższego sąsiada (Michałowo, podlaskie). Punkt najdalszy od jakiejkolwiek Żabki - pusta przestrzeń w Bieszczady - to około {{STAT_VOID_DISTANCE_KM}} km. Czysta geometria na wczytanych już danych.",
    meth_sec_pipeline: "Pipeline ETL",
    meth_pipeline_desc: "Mamy codzienny proces ETL, który pobiera dane, czyści, wzbogaca i zapisuje do bazy. Wygląda tak:",
    meth_etl_step1: "<strong>Pobranie.</strong> Pobieramy publiczny plik JSON z lokalizacjami sklepów. Jeśli API niedostępne, bierzemy z lokalnego pliku fallback z <code>data/input/</code>.",
    meth_etl_step2: "<strong>Oczyszczenie.</strong> Deduplikacja po <code>storeId</code>, normalizacja nazw miast, usuwanie <code>&lt;br&gt;</code> i kodów pocztowych z ulic, wyrzucenie danych osobowych i stałych pól.",
    meth_etl_step3: "<strong>Wzbogacenie.</strong> Każde źródło niezależnie: regiony, sąsiedztwo, płazy, parki, elewacja, GUS, paczkomaty. Błąd jednego źródła nie przerywa potoku - kolumna dostaje NULL i ETL idzie dalej. Każde zapytanie sieciowe ma retry 3 próby z odstępem 60s.",
    meth_etl_step4: "<strong>Diff.</strong> Porównujemy z poprzednim snapshotem: nowe <code>store_id</code> to sklep otwarty, zniknięte - zamknięty. Wynik zapisujemy jako historia zmian bezpośrednio w tabeli lokalizacji za pomocą pola deleted_at.",
    meth_etl_step5: "<strong>Zapis do bazy.</strong> Zapis do DuckDB. Nowe kolumny dodajemy przez <code>ALTER TABLE ADD COLUMN IF NOT EXISTS</code>.",
    meth_etl_step6: "<strong>Czyszczenie cache.</strong> Po sukcesie ETL czyścimy cache Redis. Backend odbudowuje odpowiedzi na nowym stanie. Redis jest opcjonalny - jak niedostępny (np na srodowisku lokalnym), backend działa bez niego.",
    ...
    meth_warn_item5: "Dane dot. lokalizacji Żabek obejmują <strong>jedynie aktualnie aktywne sklepy</strong>. Nie mamy więc danych historycznych - najstarsza Żabka niekoniecznie musi być najstarsza w ogóle, bo jest najstarsza <strong>z aktywnych</strong>.",
    ...
    fact_lab_isolated: "Najdalej od innej Żabki",
    fact_lab_oldest: "Najstarsza wciąż czynna",
    fact_lab_void: "Największa pustka",
    fact_lab_crown: "Korona kolekcji",
    fact_lab_frogrecord: "Rekord obserwacji płazów",
    fact_lab_farfrog: "Najdalej od obserwacji żaby",
    fact_lab_zerofrog: "Bez żadnej obserwacji żaby w pobliżu",
    ...
    fact_desc_zerofrog: "Tyle sklepów nie ma ani jednej obserwacji płaza w promieniu 5 km (GBIF, Amphibia). Najbardziej odizolowana Żabka: {isolated_store} - {distance} km od najbliższej obserwacji żaby.",
    ...
    fact_short_zerofrog: "sklepy bez obserwacji płaza w promieniu 5 km",
    ...
    // i18n coverage audit additions
    meta_title: "Żabkozbiór - interaktywny atlas Żabek",
    ...
    jsonld_website_desc: "Interaktywny atlas Żabek zbudowany na danych publicznych.",
...
// FAQPage mainEntity order on faq.html - keeps the JSON-LD text in sync with
// the visible Q&A instead of duplicating a second hardcoded copy per language.
const FAQ_JSONLD_KEYS = [
  ['faq_q_count', 'faq_a_count'],
  ['faq_q_most', 'faq_a_most'],
  ['faq_q_farthest', 'faq_a_farthest'],
  ['faq_q_yearly', 'faq_a_yearly'],
  ['faq_q_closes', 'faq_a_closes'],
  ['faq_q_source_origin', 'faq_a_source_origin'],
  ['faq_q_update_freq', 'faq_a_update_freq'],
  ['faq_q_official', 'faq_a_official'],
  ['faq_q_why', 'faq_a_why'],
  ['faq_q_econ_correlation', 'faq_a_econ_correlation'],
  ['faq_q_amphibians_density', 'faq_a_amphibians_density'],
  ['faq_q_growth_bias', 'faq_a_growth_bias'],
  ['faq_q_saturation_warsaw', 'faq_a_saturation_warsaw'],
];

function _htmlToText(htmlStr) {
  const div = document.createElement('div');
  div.innerHTML = htmlStr;
  return div.textContent.replace(/\s+/g, ' ').trim();
}

function _readJsonLd(id) {
  const script = document.getElementById(id);
  if (!script) return null;
  try { return { script, data: JSON.parse(script.textContent) }; } catch (e) { return null; }
}

function updateJsonLd() {
  const lang = getLang() === 'en' ? 'en' : 'pl-PL';

  const main = _readJsonLd('jsonld-main');
  if (main) {
    (main.data['@graph'] || []).forEach(node => {
    ...
    main.script.textContent = JSON.stringify(main.data, null, 2);
  }

  const meth = _readJsonLd('jsonld-meth');
  if (meth) {
    meth.data.headline = t('meth_title');
    meth.data.description = t('meth_meta_desc');
    meth.data.inLanguage = lang;
    meth.script.textContent = JSON.stringify(meth.data, null, 2);
  }

  const faq = _readJsonLd('jsonld-faq');
  if (faq && Array.isArray(faq.data.mainEntity)) {
    faq.data.inLanguage = lang;
    faq.data.mainEntity.forEach((item, i) => {
      const pair = FAQ_JSONLD_KEYS[i];
      if (!pair) return;
      item.name = _htmlToText(t(pair[0]));
      if (item.acceptedAnswer) item.acceptedAnswer.text = _htmlToText(t(pair[1]));
    });
    faq.script.textContent = JSON.stringify(faq.data, null, 2);
  }
}
